...
export default function CRMLayout({ children }: CRMLayoutProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { id: 'conversations', label: 'Inbox', icon: MessageCircle, href: '/conversations'},
  ...
  return (
    <div className="h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full bg-white border-r border-gray-200 transition-all duration-300 z-30 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        {/* Header */}
        ...
        <nav className="mt-6">
          {menuItems.map((item) => (
            ...
            >
              <item.icon className={`h-5 w-5 ${isActive(item.href) ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600'}`} />
              {sidebarOpen && (
                <>
