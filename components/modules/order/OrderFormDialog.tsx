...
export default function OrderFormDialog({ 
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        order_items: initialData.order_items?.length > 0 
  ...
  const validateForm = () => {
    ...
    if (formData.order_items.some(item => !item.product_id || item.quantity <= 0)) {
      newErrors.order_items = 'All items must have a valid product ID and quantity';
    }
